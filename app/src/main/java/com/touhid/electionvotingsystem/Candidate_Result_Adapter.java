...
/**
 * Created by Heskay Win 10 on 8/18/2018.
 */
...
public class Candidate_Result_Adapter extends RecyclerView.Adapter<Candidate_Result_Adapter.ViewHolder> {
   ...
   // private FirebaseAuth firebaseAuth;
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        Collections.sort(result_list, new Comparator<Candidate_Result>() {
            public int compare(Candidate_Result obj1, Candidate_Result obj2) {
                // ## Ascending order
//                            return obj1.firstName.compareToIgnoreCase(obj2.firstName); // To compare string values
        ...
        String cand_name_data = result_list.get(position).getCandidatename();
        holder.setCandnameText(cand_name_data);

        String cand_post_data = result_list.get(position).getCandidatepost();
        holder.setCandPost(cand_post_data);

        String cand_party_data = result_list.get(position).getPartyname();
        holder.setCandParty(cand_party_data);

        String cand_speech_data = result_list.get(position).getCandidatespeech();

        String image_uri = result_list.get(position).getImage_uri();
...
//                    holder.updatecount(count);
                    ...
                    Candidate_Result updatedResult = result_list.get(position);
                    updatedResult.setVoteCount(count);
                    result_list.set(position, updatedResult);
...
//                    holder.updatecount(0);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setCandnameText(String candText) {
            ...
            CandNameView_Result = mView_Result.findViewById(R.id.candname_result);
            CandNameView_Result.setText(candText);
        }

        public void setCandImage(String downloadUri) {

            CandidateImageView_Result = mView_Result.findViewById(R.id.candidate_image_result);
            Glide.with(context).load(downloadUri).into(CandidateImageView_Result);

        }

        public void setCandPost(String candPostText) {

            CandPostView_Result = mView_Result.findViewById(R.id.candidate_post_result);
            CandPostView_Result.setText(candPostText);
        }

        public void setCandParty(String candPartyText) {

            CandPartyView_Result = mView_Result.findViewById(R.id.party_name_result);
            CandPartyView_Result.setText(candPartyText);
        }

        public void updatecount(int count) {

            CandidateResultCount = mView_Result.findViewById(R.id.candidate_results);
            CandidateResultCount.setText(count + "");
